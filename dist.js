...
getAsync(host + "/available-timestamps.json", function (text) {
    ...
    function advanceImage() {
        ...
        */
        ...
        var prettyTime = imageTime.format('MMMM Do YYYY, h:mm:ss a');
...
},{"./timestamp-transforms":4,"mapbox-gl":1}],4:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var moment = require("moment");
// Transformations working on strings, going between times and radar timestamps
// (the time string in a URL)
// Converts IDR714.T.201807031106.png to IDR714
function filenameToRadarID(filename) {
    if (filename.substring(0, 3) !== "IDR") {
        throw Error("Filename " + filename + " doesn't start with IDR");
    }
    if (!filename.includes(".")) {
        throw Error("Filename ${filename} doesn't have any dots");
    }
    if ((filename.match(/\./g) || []).length !== 3) {
        throw Error("Filename ${filename} doesn't have three dots");
    }
    var segments = filename.split('.');
    if (segments[segments.length - 1] !== 'png') {
        throw Error("Filename ${filename} doesn't have png extension");
    }
    return filename.split(".")[0];
}
exports.filenameToRadarID = filenameToRadarID;
function filenameToTime(filename) {
    var radarTimestamp = filename.split(".")[2];
    if (radarTimestamp.length !== 12) {
        throw Error("Filename " + filename + " has incorrect radar timestamp length");
    }
    return radarTimestampToTime(radarTimestamp);
}
exports.filenameToTime = filenameToTime;
function timeToRadarTimestamp(time) {
    return time.utc().format('YYYYMMDDHHmm');
}
exports.timeToRadarTimestamp = timeToRadarTimestamp;
function radarTimestampToTime(radarTimestamp) {
    return moment.utc(radarTimestamp, "YYYYMMDDHHmm");
}
exports.radarTimestampToTime = radarTimestampToTime;
function buildRadarImageURL(serverURL, radarID, time) {
    var radarTimestamp = timeToRadarTimestamp(time);
    var year = time.year();
    var month = time.format("MM");
    var day = time.format("DD");
    var hour = time.format("HH");
    var filename = radarID + ".T." + radarTimestamp + ".png";
    return "" + serverURL + radarID + "/" + year + "/" + month + "/" + day + "/" + hour + "/" + filename;
}
exports.buildRadarImageURL = buildRadarImageURL;
